...
export const exportFeedbackToPDF = (
  ...
  try {
    ...
    doc.text('CG Portal Feedback System', pageWidth / 2, 20, { align: 'center' })
    ...
    doc.setFontSize(12)
    doc.text(`Department: ${department}`, pageWidth / 2, 45, { align: 'center' })
    doc.text(`Generated on: ${new Date().toLocaleDateString('en-IN')} at ${new Date().toLocaleTimeString('en-IN')}`, pageWidth / 2, 55, { align: 'center' })
    ...
    doc.setFontSize(10)
    doc.text(`Total Feedbacks: ${totalFeedbacks}`, 20, 70)
    doc.text(`Resolved: ${resolvedCount}`, 20, 80)
    doc.text(`Pending: ${pendingCount}`, 80, 80)
    doc.text(`Satisfied: ${satisfiedCount}`, 140, 80)
...
export const exportDepartmentSummary = (feedbacks: Feedback[]) => {
  try {
    ...
    doc.setFontSize(16)
    doc.setTextColor(0, 0, 0)
    doc.text('Department-wise Summary Report', pageWidth / 2, 35, { align: 'center' })
    
    doc.setFontSize(12)
    doc.text(`Generated on: ${new Date().toLocaleDateString('en-IN')} at ${new Date().toLocaleTimeString('en-IN')}`, pageWidth / 2, 50, { align: 'center' })
